...
if: (se) O if é a estrutura básica de uma condição. Ele verifica se uma 
expressão é verdadeira e, se for, executa um bloco de código associado.

else: (não) O else é usado em conjunto com o if para fornecer um bloco de código 
alternativo a ser executado quando a condição do if for falsa.

else if: (senão) O else if é usado quando há múltiplas condições a serem verificadas. 
